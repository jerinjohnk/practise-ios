...
class HomeController: UIViewController {
    ...
    func fetchUsers(forCurrentUsern user: User) {
        Service.fetchUsers(forCurrentUsern: user) { users in
    ...
    func fetchCurrentUserAndCards() {
        ...
        Service.fetchUser(withUid: uid) { user in
            self.user = user
            self.fetchUsers(forCurrentUsern: user)
        }
    }
    ...
    func performSwipeAnimation(shouldLike: Bool) {
        ...
        UIView.animate(withDuration: 1.9, delay: 0, usingSpringWithDamping: 0.6, initialSpringVelocity: 0.1, options: .curveEaseOut, animations: {
...
extension HomeController: CardViewDelegate {
    func cardView(_ view: CardView, didLikeUser: Bool) {
        ...
        saveSwipeAndCheckForMatch(forUser:
            user, didLike: didLikeUser)
...
extension HomeController: BottomControlsStackViewDelegate {
    ...
    func handleLike() {
        ...
        saveSwipeAndCheckForMatch(forUser:
            topCard.viewModel.user, didLike: true)
    }
...
extension HomeController: MatchViewDelegate {
    func matchView(_ view: MatchView, wantsToSendMessageTo user: User) {
        print("DEBUG: send message to \(user.name)")
    }
